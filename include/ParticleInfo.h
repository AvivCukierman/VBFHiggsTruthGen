...
class ParticleInfo : public PseudoJet::UserInfoBase{
 ...
 ParticleInfo(const int & pdg_id_in,
            const int & pythia_status_in,
	    const int & pythia_id_in,  
	    const double & pv_in, 
	    const bool & pileup_in,
	    const double & pt_in) 
   :_pdg_id(pdg_id_in),
    _pythia_status(pythia_status_in), 
  ...
  int pythia_status() const { return _pythia_status;}
 ...
 protected:
  ...
  int _pythia_status;         // status in pythia.event (http://home.thep.lu.se/~torbjorn/pythia81html/ParticleProperties.html) 
