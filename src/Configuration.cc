...
void Configuration::ConfigurePythiaSignal(Pythia8::Pythia* hs){
  ...
  if(proc ==1){
    ...
    hs->readString("Zprime:ad = 0");
    hs->readString("Zprime:au = 0");
    hs->readString("Zprime:ae = 0");
    hs->readString("Zprime:anue = 0");
    hs->readString("Zprime:ve = 0");
    hs->readString("Zprime:vnue = 0");
    
    double cosThetaW = 80.385/91.1876;
    double pre_factor = 4*cosThetaW/0.626;
    double gB = 1;
    pre_factor*=gB/6.;
    std::stringstream ss_vd; ss_vd << "Zprime:vd =  " << pre_factor;
    std::stringstream ss_vu; ss_vu << "Zprime:vu =  " << pre_factor;
    hs->readString(ss_vd.str());
    hs->readString(ss_vu.str());
